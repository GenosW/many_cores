 ...
 */
 void solve_system(int N, int M) {
  ...
  Timer timer;
  const int size = N * M; 
  int unknowns = N*3 - 2;// number of unknows to solve for --> to check
#ifdef DEBUG
...
int main() 
{
  ...
  std::vector<size_t> p_per_dir{(size_t)sqrt(5)}; //{ (size_t)sqrt(1e3), (size_t)sqrt(1e4), (size_t)sqrt(1e5), (size_t)sqrt(1e6), (size_t)sqrt(4e6)};
