...
"  }"
"  //double val = work_group_reduce_add(dot);\n"
"\n"
"  if (lid == 0) {\n"
"    //atomic_add(result, val);\n"
"    //my_atomic_add(result, val);\n"
"    my_atomic_add(result, cache[0]);\n"
"  }\n"
"}";
...
int main()
{
  ...
  //
  cl_kernel my_kernel = clCreateKernel(prog, "xDOTy", &err); OPENCL_ERR_CHECK(err);
  ...
  cl_mem ocl_dot = clCreateBuffer(my_context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, sizeof(ScalarType), &(dot), &err); OPENCL_ERR_CHECK(err);
  ...
  err = clEnqueueReadBuffer(my_queue, ocl_dot, CL_TRUE, 0, sizeof(ScalarType), &dot, 0, NULL, NULL); OPENCL_ERR_CHECK(err);

  std::cout << std::endl;
  std::cout << "Result of kernel execution: " << dot << std::endl;
